 ...
 *   LSP  — all functions return the same OrgUnit shape as a raw Mongoose query.
 *
...
export async function getChurchesForConference(conferenceCode, projection = null) {
  ...
  const queryOpts = projection ? { lean: true } : { lean: true }
...
export async function assertChurchInConference(churchCode, conferenceCode) {
  ...
  const codes = new Set(churches.map(c => c.code))
