 ...
 *
 * Usage:
 *   node scripts/migrateEntityAliases.js
 *   DRY_RUN=true node scripts/migrateEntityAliases.js
 *
 ...
 * Priority: protected list > Yearbook match > shorter code >
 *           not C-numeric (C10174) > not suffix-digit (MVG2) >
 *           most YearlyStats > alphabetical
 */
function selectCanonical(codes, statsCountMap) {
  // Protected codes always win
...
async function main() {
  ...
  // Load all visible (not yet hidden) OrgUnits — skip already-processed hidden ones
  ...
  // Get YearlyStats counts per code
  ...
  // Find duplicate groups: entities with the same normalised name + level
  const normalise = (s) => s ? s.toLowerCase().replace(/[^a-z0-9]/g, '') : ''
  const groups = new Map() // normKey → [code, ...]
  ...
  for (const codes of duplicateGroups) {
    ...
    // Update canonical: add aliases array
    bulkOrgUnit.push({
      updateOne: {
    ...
    for (const alias of aliases) {
      ...
      // Migrate YearlyStats from alias to canonical
      ...
      for (const stat of aliasStats) {
        // Check if canonical already has a record for this year
